...
| 路线 | 怎么做 | 手机能用吗 | 费用 | 耗时 |
|---|---|---|---|---|
| **A. 纯 IP 访问** | 服务器跑服务 + Nginx 接管 80，直接访问 `http://IP`（无端口号，前端由服务端托管，同源） | ✅ 能 | 0（只有服务器钱） | 1 小时 |
| ~~B. 域名 + 非标端口免备案~~ | ❌ **此路不通**，见下方更正 | — | — | — |
| **C. 域名 + 备案 + 80/443** | 走标准姿势（唯一能让域名可用的路） | ✅ 能，且 HTTPS | +域名 60~80 元 | 1~3 周（备案） |

**建议**：先走 A（当天就能用），域名等备案下来再切。

> ### ⚠️ 实测更正（2026-09-14）：非标端口躲不过备案拦截
>
> 本手册原先写"非标端口不在拦截范围内" —— **这是错的**。实测：域名解析到阿里云国内服务器后，
> `http://域名:3457`、`http://域名:908` 全部返回 **403**，响应头 `Server: Beaver`、页面标题
> `Non-compliance ICP Filing`（阿里云备案拦截页）；而**同一台服务器的 IP 访问（任意端口）完全正常**。
>
> **结论**：阿里云按**域名（Host 头）**拦截，与端口号无关。
> - 想让域名可用 → **只能备案**
> - 不想备案 → **用 IP 访问**（配合 Nginx 监听 80，地址可以不带端口）
...
**想跳过备案？不行。** ~~用非标端口可以绕过~~ —— 这是本手册早期的错误说法，已由 2026-09-14 实测推翻：
阿里云按域名（Host 头）拦截，`http://域名:任意端口` 都被拦（403 / `Server: Beaver`）。
**不做备案，域名就用不了**；替代方案只有用 IP 访问（见第 0 节的更正说明）。
