...
| 首屏 | 体积守住：`index.html` 457 KB raw → 144.0 KB gzip 传输（省 68%） |
...
前端是**一个 8,511 行的 `index.html`**，后端是**一个 946 行的 `server.js`**，
...
完整复审 + 修复，全部有回归测试。**334 项断言**（8 个套件）+ 3 套冒烟脚本。
...
| 前端仍是单文件 8,511 行 | 有意保留，见第七节「如果重来」 |
...
```
nexus-core/
├── index.html              前端全部（8,511 行 / 457 KB）
├── README.md               使用与部署说明
...
### 测试
```bash
cd server && npm test          # 334 项断言（8 个套件）
